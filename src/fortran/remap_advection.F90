...
!BOP
!
! !MODULE: remap_advection - horizontal transport via incremental remapping
!
! !DESCRIPTION:
!
...
!
! 2006: Created this module by modifying the remapping module in CICE 4.0
! 2009: Adapted for GLIMMER/CISM
!
! !INTERFACE:
!
      module remap_advection
!
! !USES:
!
...
!lipscomb - test points for debugging
      public :: mtest, itest, jtest, ktest, btest
...
!
      ...
      real (kind=dp), intent(inout),     &
         dimension (nx_block,ny_block,ntrace,max_blocks) ::     &
         tm           ! mean tracer values in each grid cell

      logical :: l_prescribed_area = .false.
      logical :: l_prescribed_mass = .false.
      ...
      ! NOTE: These limiting options are only for the mass field
      !       Would need more infrastructure to apply these options to tracers
      logical, intent(in), optional ::             &
         l_min_only_in, &! if true, limit mins only (for mass field)
         l_max_only_in, &! if true, limit maxes only (for mass field)
         l_posdef_only_in   ! if true, limit mass only for positive definiteness
      ...
      real (kind=dp), dimension (nx_block,ny_block,max_blocks),   &
         ...
         tarea_r          ! reciprocal grid cell area (m-2)
...
!
      ...
      real (kind=dp), dimension (nx_block,ny_block,max_blocks) ::  &
         ...
         xxxav, xxyav            ,&! mean T-cell values of xxx, xxy
         xyyav, yyyav              ! mean T-cell values of xyy, yyy

      integer, dimension (ntrace) ::     &
         tracer_type       ,&! = 1, 2, or 3 (see comments above)
         depend              ! tracer dependencies (see above)

      logical ::   &
         l_min_only,       &! if true, limit mins only (for mass field)
         l_max_only,       &! if true, limit maxes only (for mass field)
         l_posdef_only      ! if true, limit mass only for positive definiteness

      real (kind=dp), dimension(nx_block,ny_block,max_blocks) ::  &
         mass_e         ,&! mass at east edges
         mass_n           ! mass at north edges

      logical, dimension (ntrace) ::     &
      ...
      real (kind=dp), dimension (nx_block,ny_block,ntrace) ::     &
      ...
      integer, dimension(ngroups,max_blocks) ::     &
...
!!!      type (bndy) ::     &!  ghost cell update info
!!!         bndy_info        ! block distribution
...
!     istep1 = nsteps_run
    ...
    !  default values.
    ! Note: The default values for xav, etc. are for a rectangular cell
    ...
    !------------------------------------------------------------------- 
      ...
      if (present(l_max_only_in)) then
         l_max_only = l_max_only_in
      else
         l_max_only = .false.
      endif

      if (present(l_min_only_in)) then
         l_min_only = l_min_only_in
      else
         l_min_only = .false.
      endif

      if (present(l_posdef_only_in)) then
         l_posdef_only = l_posdef_only_in
      else
         l_posdef_only = .false.
      endif
...
!!!                                  this_block%j_glob(jstop) 
    ...
    !-------------------------------------------------------------------
    ! Ghost cell updates for departure points, centroids and gradients
    ! These are not needed if remapping is done inside a block loop
    ! (which requires two layers of ghost cells.)
    ! Uncomment if block loops are inside the remapping.
    !-------------------------------------------------------------------
...
!!!                               field_loc_center, field_type_vector)
    ...
    !-------------------------------------------------------------------
         ...
         if (my_task==mtest .and. verbose) then
            ...
            h2 = dyt(i,j,iblk)
            print*, ''
            print*, 'East fluxes, m, i, j =', my_task, i, j
            print*, ''
            print*, 'dp(i,j) =', dpx(i,j,iblk), dpy(i,j,iblk)
            print*, 'dp(i,j-1) =', dpx(i,j-1,iblk), dpy(i,j-1,iblk)
            print*, ''
            print*, 'h1 =', h1
            print*, 'h2 =', h2
            print*, ''
            do ng = 1, ngroups
               if (abs(triarea(i,j,ng)) > puny) then
                  print*, ' '
                  print*, 'East triangle: ng =', ng
                  print*, 'Triarea =', triarea(i,j,ng)
                  print*, 'iflux jflux =', iflux(i,j,ng), jflux(i,j,ng)
                  print*, 'Vertices:'
                  print*, 1, xp(i,j,1,ng), yp(i,j,1,ng)
                  print*, 2, xp(i,j,2,ng), yp(i,j,2,ng)
                  print*, 3, xp(i,j,3,ng), yp(i,j,3,ng)
                  print*, 'xp2 - xp1 =', xp(i,j,2,ng) - xp(i,j,1,ng),   &
                                         yp(i,j,2,ng) - yp(i,j,1,ng)
                  print*, 'xp3 - xp1 =', xp(i,j,3,ng) - xp(i,j,1,ng),   &
    ...
    !-------------------------------------------------------------------
         ...
         call locate_triangles(nx_block,          ny_block,           &
                               ...
                               iflux,             jflux,              &
                               triarea,                               &
                               l_prescribed_area, edgearea_n(:,:,iblk))
         ...
         if (my_task==mtest .and. verbose) then
            ...
            h2 = dyt(i,j,iblk)
            print*, ''
            print*, 'North fluxes, m, i, j =', my_task, i, j
            print*, ''
            print*, 'dp(i-1,j) =', dpx(i-1,j,iblk), dpy(i-1,j,iblk)
            print*, 'dp(i,j) =', dpx(i,j,iblk), dpy(i,j,iblk)
            print*, ''
            print*, 'h1 =', h1
            print*, 'h2 =', h2
         ...
         call transport_integrals(nx_block,           ny_block,           &
                                  ...
                                  tc(:,:,:,iblk),     tx    (:,:,:,iblk), &
                                  ty(:,:,:,iblk),     mtflxn(:,:,:),      &
                                  l_prescribed_mass,  mass_n(:,:,  iblk) )
 
    !-------------------------------------------------------------------
    ! Update the mass and tracers.
    !-------------------------------------------------------------------
...
!
    ...
    !-------------------------------------------------------------------
    ! Find grid cells where mass is nonzero.
    !-------------------------------------------------------------------
...
!
      ...
      logical, intent(in), optional ::              &
          l_min_only_in,   &! if true, limit mins only
          l_max_only_in,   &! if true, limit maxes only
          l_posdef_only_in  ! if true, limit only for positive definiteness
!
!EOP
!
      integer ::   &
      ...
      logical ::              &
          l_min_only,      &! if true, limit mins only
          l_max_only,      &! if true, limit maxes only
          l_posdef_only     ! if true, limit only for positive definiteness
...
!
      ...
      do ij = 1, icells
         ...
         gxtmp = p5 * (phi_e - phi_w)
         gytmp = p5 * (phi_n - phi_s)
! The following would be used if coordinates were not scaled to unit length
...
!               + (phi(i,j) - phi_s) / (dyt(i,j-1) + dyt(i,j)  )
         ...
         pmn = pmn - phi(i,j)   ! pmn <= 0
         pmx = pmx - phi(i,j)   ! pmx >= 0

         ! minimum and maximum deviation of phi within the cell
         ! Note:  cell edges have unit length in scaled coordinates
         w1  =  (p5 - cnx(i,j)) * gxtmp   &
              + (p5 - cny(i,j)) * gytmp
         w2  =  (p5 - cnx(i,j)) * gxtmp   &
              - (p5 + cny(i,j)) * gytmp
         ...
         w4  =  (p5 - cny(i,j)) * gytmp   &
          ...
          ! if cell edges were not scaled to unit length
...
!              - (p5*htn(i,j)   + cnx(i,j)) * gxtmp
         ...
         qmn = min (w1, w2, w3, w4)  ! qmn <= 0
         qmx = max (w1, w2, w3, w4)  ! qmx >= 0

         if (l_min_only) pmx = qmx  ! no limiting on max side
 
         if (l_max_only) pmn = qmn  ! no limiting on min side

         if (l_posdef_only) then
            pmx = qmx         ! no limiting on max side
            pmn = -phi(i,j)   ! reconstructed mass must be >= 0
         endif

         ! the limiting coefficient

         if (abs(qmn) > c0) then 
            ! 'abs(qmn) > puny' not sufficient
            w1 = max(c0, pmn/qmn)
         else
            w1 = c1
         endif
...
!
    ...
    !-------------------------------------------------------------------
      ...
      if (l_stop) then
         ...
         write (nu_diag,*) 'uvel, vvel =', uvel(i,j), vvel(i,j)
         return
      endif

!lipscomb - to be added later
      if (l_dp_midpt) then ! find dep pts using corrected midpt velocity 
         write (nu_diag,*) 'Midpoint method not implemented in remapping'
         l_stop = .true.
      endif
...
!
      ...
      real (kind=dp), dimension(nx_block,ny_block), intent(in) ::  &
         dpx            ,&! x coordinates of departure points at cell corners
         dpy            ,&! y coordinates of departure points at cell corners
         dxu            ,&! x dimension of U-cell (m)
         dyu              ! y dimension of U-cell (m)
      ...
      logical, intent(in) ::   &
         l_prescribed_area  ! if true, the area of each departure region is
...
!
      ...
      real (kind=dp) ::   &
         ...
         xdm, ydm       ,&! midpoint of segment connecting DL and DR;
                          ! shifted if l_prescribed_area = T
         dxc            ,&! xcr - xcl
         dxd            ,&! xdr - xdl
    ...
    !-------------------------------------------------------------------
      ...
      do ij = 1, icellsd
         ...
         ! For l_prescribed_area = T, shift the midpoint so the departure
         ...
         if (my_task==mtest .and. i==itest .and. j==jtest .and. verbose) then
            print*, ''
            print*, 'Geometry, edge =', trim(edge)
            print*, 'm, i, j:', my_task, i, j
            print*, ''
            print*, 'dxu(i,j) =', dxu(i,j)
            print*, 'dyu(i,j) =', dyu(i,j)
            print*, ''
            print*, 'cl =', xcl, ycl
            print*, 'cr =', xcr, ycr
            print*, ''
            ...
            if (l_prescribed_area) then
               print*, ''
               print*, 'Scaled edgearea =', edgearea(i,j)/areafac(i,j)
               print*, 'displacement length =', w2
            endif
         endif
    ...
    ! If l_prescribed_area = T, this area is compared to edgearea as
    !  a bug check.
    ! If l_prescribed_area = F, this area is passed as an output array.
    !-------------------------------------------------------------------
      ...
      do ng = 1, ngroups
         ...
         do ij = 1, icellsd
            ...
            if (abs(triarea(i,j,ng)) < eps13*areafac(i,j)) then
               triarea(i,j,ng) = c0
            else
      ...
      if (l_prescribed_area) then
       if (bugcheck) then   ! set bugcheck = F to speed up code
         do ij = 1, icellsd
            i = indxid(ij)
            j = indxjd(ij)
            if (abs(areasum(i,j) - edgearea(i,j)) > puny*areafac(i,j)) then
               print*, ''
               print*, 'Areas do not add up: m, i, j, edge =',   &
                        my_task, i, j, trim(edge)
               print*, 'edgearea =', edgearea(i,j)
               print*, 'areasum =', areasum(i,j)
               print*, 'error =', abs(areasum(i,j) - edgearea(i,j))
               print*, 'error tolerance =', puny*areafac(i,j)
               print*, 'Triangle areas:'
               do ng = 1, ngroups   ! not vector friendly
                  if (abs(triarea(i,j,ng)) > puny*areafac(i,j)) then
...
!
      ...
      logical ::    &
           l_prescribed_mass  ! if true, masses at each edge are prescribed

      real (kind=dp), dimension(nx_block,ny_block),             &
           intent(in), optional ::                                    &
           mass               ! prescribed mass at edge
...
!ocl novrec      !Fujitsu
            do ij = 1, icells(ng)
               ...
               ! prescribe masses if desired

               if (l_prescribed_mass) then
                  mc(i2,j2) = mass(i,j)
                  mx(i2,j2) = c0
                  my(i2,j2) = c0
               endif
...
!
    ...
    !-------------------------------------------------------------------
      ...
      if (l_stop) then
         ...
         write (nu_diag,*) 'Old mass =', mm(i,j) + w1*tarea_r(i,j)
         write (nu_diag,*) 'New mass =', mm(i,j)
         write (nu_diag,*) 'Net transport =', -w1*tarea_r(i,j)
         return
      endif
         ...
         do nt = 1, ntrace
            ...
            if (tracer_type(nt)==1) then ! does not depend on other tracers
               ...
               do ij = 1, icells
                  ...
                  tm(i,j,nt) = (mtold(i,j,nt) - w1*tarea_r(i,j))   &
                                / mm(i,j)
               enddo            ! ij
